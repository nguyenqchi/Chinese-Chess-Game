...
class BoardGame:
    ...
    def lordTolord(self):
        ...
        if col1 == col2:
            ...
            return False
    def getActiveRed(self):
        """
        Get all active pieces of the red side
        """
        activeRed = []
        for piece in self.activePices:
            if piece.side == RED_SIDE:
                activeRed.append(piece)
        return activeRed
    def getActiveBlue(self):    
        """
        Get all active pieces of the blue side
        """
        activeBlue = []
        for piece in self.activePices:
            if piece.side == BLUE_SIDE:
                activeBlue.append(piece)
        return activeBlue

    def getActivePieces(self, side):
        """
        Get all active pieces
        """
        if side == RED_SIDE:
            return self.getActiveRed()
        else:
            return self.getActiveBlue()
    ...
    def isOver(self):
        ...
        """
        if self.redLord.mated:
            return True
        elif self.blueLord.mated:
            return True
        elif self.lordTolord():
            return True
        else:
            return False